                    ...
                    if (result._STATUS_ == 'FAIL') {
                        ...
                        if (data.length == undefined || data.length == 0) {
                            ...
                            for (i = 0; i < data.length; i++) {
                                ...
                                $("#hdnSalutation").val(data[i].chrNameInitial.trim());
                ...
                if (result._STATUS_ == 'FAIL') {
                    ...
                    if (data.length == undefined || data.length == 0) {
                        ...
                        if (DistrictId != undefined) {
                            ...
                            $("#cboDistricts_current").trigger("change", [Pincode, ExamCenter]);
    ...
    var validator = $("#frmMain").validate(
        {
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    enctype: 'multipart/form-data',
                    url: "../Candidates/SaveDuplicateURN",
                    data: data,
                    processData: false,
                    contentType: false,
                    cache: false,
                    success: function (msg) {
                        ...
                        if (Result._STATUS_ == "SUCCESS") {
                            var x = '<H4>' + Result._MESSAGE_ + '<br>' + 'The New URN no generated is : ' + Result.URN + '</H4>';
                            $("#Result").html(x);
                            $("#Result").show();
                            form.reset();
                            $("#Form").hide();
    ...
    $("#frmUploadMain").validate(
        {
            ...
            messages: {
                txtFile: {
                    ...
                    filesize: "File size must be less than 500 kilobytes"
                },
                chkDeclarationU: {
                    required: 'You must confirm the declaration by checking the check box',
                }
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    enctype: 'multipart/form-data',
                    url: "../Candidates/UploadDuplicateURN",
                    data: data,
                    processData: false,
                    contentType: false,
                    cache: false,
