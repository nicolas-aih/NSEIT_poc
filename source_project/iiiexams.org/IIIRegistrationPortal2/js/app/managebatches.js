$(document).ready(function () {
    ...
    $("#txtFromDate").on('keyup keypress blur change', function () {
        $('#hdBatchNo').val('');
        $('#hdAmount').val('');
        //$("#tbldata").html('');
        $("#tblBatches").html('');
    ...
    $("#txtToDate").on('keyup keypress blur change', function () {
        $('#hdBatchNo').val('');
        $('#hdAmount').val('');
        //$("#tbldata").html('');
        $("#tblBatches").html('');
            ...
            success: function (msg) {
                ...
                if (result._STATUS_ === 'FAIL') {
                    ...
                    if (data.length == undefined || data.length === 0) {
                        ...
                        $('#paymentstatus').html(result.PaymentStatus);
                        ...
                        if (result.payment_mode.toUpperCase() === 'CREDIT') {
                            s2 = "<thead><tr><th>Payment details</th></tr>";
                            s2 += "<tr><th>Payment Date</th></tr>";
                            s2 += "<tbody>";
                            s2 += "<tr><td>" + result.PaymentDate + "</td></tr>";
                            s2 += "</tbody>";
                        }
                        else {
    ...
    $("#btnDeleteBatch").click(function () {
        if ($("#hdBatchNo").val() === "") {
            alert("Please enter the batch no / transaction id");
            return;
        }
        if (!confirm("Do you really want to proceed with deletion of this batch ?")) {
            return;
        }
        var data = new FormData();
        data.append("hdBatchNo", $("#hdBatchNo").val());
        $.ajax({
            type: "POST",
            enctype: 'multipart/form-data',
            url: "../Services/DeleteBatch",
            data: data,
            processData: false,
            contentType: false,
            cache: false,
    ...
    $("#btnDeleteProblemURNS").click(function () {
        if ($("#hdBatchNo").val() === "") {
            alert("Please enter the batch no / transaction id");
            return;
        }
        if (!confirm("Do you really want to proceed with deletion of problem urns from this batch ?")) {
            return;
        }
        var data = new FormData();
        data.append("hdBatchNo", $("#hdBatchNo").val());
    ...
    $("#btnMakePayment2").click(function () {
        if ($("#selPG option:selected").val() === "")
        {
            alert("Please select the Payment gateway, to proceed with online payment.");
            return; 
        }
        $("#divPGSel").dialog('close');

        var data = new FormData();
        data.append("hdBatchNo", $("#hdBatchNo").val());
        data.append("selPG", $("#selPG option:selected").val());
        $.ajax({
            type: "POST",
            enctype: 'multipart/form-data',
            url: "../Services/VerifyBatch",
            data: data,
            processData: false,
            contentType: false,
            cache: false,
            success: function (msg) {
                ...
                var result = JSON.parse(msg);
                if (result._STATUS_ === 'SUCCESS') {
                    if (result.redirect_to) {//pg
                        window.open(result.redirect_to, "_pg");
                    }
                    else {//non PG
                        alert(result._MESSAGE_);
                        //add code here to referesh the grid.
                        $("#frmSearch").submit();
                    }
                }
                else {
    ...
    $("#btnClosePaymentDlg").click(function () {
        $("#divPGSel").dialog('close');
    });
